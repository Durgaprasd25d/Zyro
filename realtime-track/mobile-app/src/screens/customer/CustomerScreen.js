...
// import AnimatedMarker from '../../components/AnimatedMarker'; // Mapbox handles markers natively
...
export default function CustomerScreen({ route, navigation }) {
  ...
  const { rideStatus, setRideStatus, pickupLocation, setPickupLocation } =
    useLocationStore();
  ...
  const handleCancelRide = async (reason) => {
    try {
      ...
      Alert.alert(
        "Network Error",
        "Unable to process cancellation at this time.",
      );
    }
  };
  ...
  const handlePaymentChange = async (method) => {
    try {
      const response = await fetch(
        `${config.BACKEND_URL}/api/ride/update-payment-method`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      Alert.alert(
        "System Error",
        "Unable to synchronization payment preference",
      );
    }
  };
  ...
  useEffect(() => {
    ...
    if (socket) {
      ...
      socket.on("payment:success", (data) => {
        ...
        Alert.alert(
          "Payment Secured",
          "Transaction successful. Your completion code is now active.",
        );
      });
      ...
      socket.on("ride:cancelled", (data) => {
        ...
        Alert.alert(
          "Booking Cancelled",
          data.reason || "The booking has been cancelled.",
        );
        setTimeout(() => navigation.navigate("Home"), 2500);
      });
      ...
      socket.on("technician:cancelled", (data) => {
        ...
        Alert.alert(
          ...
          data.message ||
            "Your assigned technician had to cancel. We're finding you a replacement immediately.",
          [
            {
              text: "Cancel Booking",
              style: "destructive",
              onPress: () =>
                handleCancelRide("Technician cancelled, no replacement wanted"),
            },
            {
  ...
  const getStatusTheme = () => {
    switch (rideStatus) {
      ...
      case "ARRIVED":
        return { color: COLORS.indigo, label: "Arrived" };
      case "IN_PROGRESS":
        return { color: "#f59e0b", label: "Working" };
      default:
        return { color: COLORS.slate, label: rideStatus || "Processing" };
    }
  };
    ...
    <View style={styles.container}>
      ...
      >
        <MapboxGL.Camera
          ...
          zoomLevel={14}
          centerCoordinate={[
            pickupLocation?.longitude || 77.209,
            pickupLocation?.latitude || 28.6139,
          ]}
        />

        {!assignedTech &&
          onlineTechnicians.map(
            (tech) =>
              tech.location && (
                <MapboxGL.PointAnnotation
                  key={tech.id}
                  id={tech.id.toString()}
                  coordinate={[tech.location.lng, tech.location.lat]}
                >
        ...
        {currentLocation && assignedTech && (
          ...
          >
            <View style={styles.techMarker}>
              <Ionicons name="car" size={24} color={COLORS.indigo} />
            </View>
          </MapboxGL.PointAnnotation>
        )}
        ...
        <View style={styles.statusPill}>
          <View
            style={[
              ...
              {
                backgroundColor:
                  connectionStatus === "connected" ? "#22c55e" : "#f59e0b",
              },
            ]}
          />
      ...
      <View style={styles.floatingControls}>
        ...
        {assignedTech && (
          <TouchableOpacity
            style={[styles.controlBtn, { backgroundColor: COLORS.indigo }]}
            onPress={() => {
              // Placeholder: External navigation or modal
              Alert.alert("Navigation", "Navigation initiated!");
            }}
          >
        ...
        >
          ...
          <View style={styles.cardHeader}>
            <View>
              ...
              <Text style={styles.jobIdValue}>
                #{rideId?.substring(rideId.length - 8).toUpperCase()}
              </Text>
            </View>
            <View
              style={[
                styles.statusBadge,
                { backgroundColor: statusTheme.color + "15" },
              ]}
            >
              <Text
                style={[styles.statusBadgeText, { color: statusTheme.color }]}
              >
                {statusTheme.label}
              </Text>
            </View>
          ...
          <View style={styles.expertRow}>
            ...
            <View style={styles.expertInfo}>
              <Text style={styles.expertName}>
                {assignedTech?.name || "Searching for Expert..."}
              </Text>
              <View style={styles.ratingRow}>
                <Ionicons name="star" size={14} color="#f59e0b" />
                <Text style={styles.ratingValue}>
                  {assignedTech?.rating || "4.9"}
                </Text>
            ...
            (rideStatus === "ACCEPTED" || rideStatus === "STARTED") && (
              <View style={styles.otpSection}>
                ...
                <Text style={styles.otpHint}>
                  Share this only when the expert reaches your door.
                </Text>
              </View>
            )}
          ...
          {mainOtp &&
            ...
            (paymentTiming === "PREPAID" || paymentStatus === "PAID") && (
              <View style={styles.otpSection}>
                <View
                  style={[styles.otpBox, { backgroundColor: COLORS.indigo }]}
                >
                  <Text
                    style={[
                      styles.otpBoxLabel,
                  ...
                  <Text style={[styles.otpBoxValue, { color: "#fff" }]}>
                    {mainOtp}
                  </Text>
                </View>
                <Text style={styles.otpHint}>
                  Provide this to finalize the service and release payment.
                </Text>
              </View>
            )}
          ...
          <View style={styles.footerInfo}>
            <TouchableOpacity
              style={[
                ...
                (paymentStatus === "PAID" || paymentTiming === "PREPAID") &&
                  styles.paymentLocked,
              ]}
              onPress={() =>
                paymentStatus !== "PAID" && setShowPaymentModal(true)
              }
        ...
        <View style={styles.modalBackdrop}>
          <View style={styles.modalSheet}>
            ...
            <TouchableOpacity
              style={[
                styles.methodItem,
                paymentMethod === "COD" && styles.methodItemActive,
              ]}
            ...
            >
              <View
                style={[
                  styles.methodIcon,
                  paymentMethod === "COD" && { backgroundColor: "#fff" },
                ]}
              >
                <Ionicons
                  name="cash"
                  size={24}
                  color={paymentMethod === "COD" ? COLORS.indigo : COLORS.slate}
                />
              </View>
              <Text
                style={[
                  styles.methodName,
                  paymentMethod === "COD" && { color: "#fff" },
                ]}
              >
            ...
            >
              <View
                style={[
                  styles.methodIcon,
                  paymentMethod === "ONLINE" && { backgroundColor: "#fff" },
                ]}
              >
                <Ionicons
                  name="card"
                  size={24}
                  color={
                    paymentMethod === "ONLINE" ? COLORS.indigo : COLORS.slate
                  }
                />
              </View>
              <Text
...
// UI and styles handled by Mapbox
